...
#define NUM_MAG_CHANNELS 8
/* Number of samples to take from a magnetometer channel */
#define NUM_MAG_SAMPLES 833
/* Magnetometer buffer length in uint16_t's */
...
static void getField(uint16_t *buf);
...
{
  int degC, volt;
  uint8_t msg[7 + (MAG_BUFFER_LEN * sizeof(uint16_t))];
...
//  volt = (resval*25)/512;
  ...
  uint16_t dtcbuf[MAG_BUFFER_LEN];
  getField(dtcbuf);
  ...
  */
  ...
  msg[4] = seqno & 0xFF;
  msg[5] = (seqno >> 8) & 0xFF;
  msg[6] = 0;  // this is also set below when APP_AUTO_ACK is TRUE and an ack is requested

  memcpy((char *) msg, (char *) dtcbuf, sizeof(dtcbuf));
...
static void getField(uint16_t *buf)
{
  ...
  static int channels[] = {INCH_0,INCH_1,INCH_2,INCH_3,INCH_4,INCH_12,INCH_13,INCH_14};
  ...
  for (i = 0; i < NUM_MAG_CHANNELS; i++) {
    ...
    // 5
    ADC10CTL0 |= ADC10SC + ENC;               // Sampling and conversion start
    ...
    while (!stop) {
      ...
      // 3
      ADC10SA = *buf + bufoffset;             // this starts the DTC
    ...
    ADC10CTL0 &= ~ENC;                        // stop ADC, go to next channel
  }
}
