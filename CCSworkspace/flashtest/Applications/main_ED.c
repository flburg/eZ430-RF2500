...
#define TRANSMIT_PERIOD_SECS 2
/* ADC buffer and payload length in unsigned short's */
#define ADC_BUFFER_LEN 255
...
BSP_TOGGLE_LED1();
  ...
  ADC10CTL1 = channels[chan] | ADC10SSEL_2;
  ADC10AE0 = 0x1f;
  ADC10AE1 = 0x70;
  __delay_cycles(240);
  ADC10CTL0 |= ENC;
  ...
  for (i = 0; i < 4608/2; ++i) {
	...
	for (j = 0; j < 8; j+=8) {
      ...
      result |= ((ADC10MEM >> 2) & 0xff) << j;                  // Retrieve result
	}
//    flash_write(0x8000+i, result);
  }
